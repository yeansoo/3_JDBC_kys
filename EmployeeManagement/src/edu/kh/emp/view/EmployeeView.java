...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				sc.nextLine();  // 입력버퍼에 남아있는 잘못 입력된 문자열 제거
								// 무한반복 방지
			}finally {
	...
	public void printAll(List<Employee> empList) {
	      if(empList.isEmpty()) {
	          ...
	          for(Employee emp : empList) { 
	             System.out.printf(" %2d  | %4s | %s | %20s | %11s |  %6s  | %3s | %d\n",
	...
	public void updateEmployee() {
		...
		int result=dao.updateEmployee(emp); // UPDATE(DML)-> 반영된 행의 개수 반환(int 형)
	...
	public void selectDeptEmp() {
		...
		if(empList.isEmpty()) System.out.println("일치하는 부서가 없습니다.");
		else printAll(empList);
	...
	public void selectSalaryEmp() {
		...
		List<Employee> empList=dao.selectSalaryEmp(inputSal);
		
		if(empList.isEmpty()) System.out.printf("%d원 이상 받는 사원이 없습니다\n",inputSal);
		else printAll(empList);
